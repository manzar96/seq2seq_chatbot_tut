...
from slp.config import semeval
...
class SemevalTask7Annotate(object):
    def __init__(self, original=True):
        self.match_token = re.compile(r'<(.*?)/>')
        self.annotation = (semeval.SPECIAL_TOKENS.UNFUNNY.value
                           if original
                           else semeval.SPECIAL_TOKENS.FUNNY.value)

    def __call__(self, x):
        return re.sub(self.match_token, fr'{self.annotation} \1', x)
